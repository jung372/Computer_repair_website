...
test("calculates both VAT amounts and technician income for card payments", () => {
  ...
  assert.deepEqual(deriveSettlement("카드 결제", 1_100_000, 100_000), {
...
test("sets total VAT to zero for cash payments", () => {
  assert.deepEqual(deriveSettlement("현금 결제", 1_100_000, 100_000), {
    totalVatAmount: 0,
    materialVatAmount: 10_000,
    technicianIncome: 990_000,
    officeDeposit: 0,
  });
});

test("calculates cash receipt VAT in the same way as card VAT", () => {
  assert.deepEqual(
    deriveSettlement("현금영수증 결제", 1_100_000, 100_000),
    deriveSettlement("카드 결제", 1_100_000, 100_000),
  );
});

test("rounds both VAT amounts to the nearest won", () => {
  // 100,000 / 11 = 9090.909... → 9,091
  const settlement = deriveSettlement("카드 결제", 100_000, 100_005);
...
test("never returns a negative technician income when material cost is too high", () => {
  assert.deepEqual(deriveSettlement("카드 결제", 110_000, 500_000), {
...
test("keeps the derived amounts consistent with the total", () => {
  for (const [total, material] of [
    ...
    const {
      ...
      technicianIncome,
      officeDeposit,
    } = deriveSettlement("카드 결제", total, material);
    assert.equal(
