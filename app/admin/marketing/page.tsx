...
import { MarketingStatusRefresh } from "@/components/marketing-status-refresh";
...
const STATUS: Record<string, string> = {
  ...
  CANCELLED: "취소", COMPLETED: "완료", PUBLISH_UNKNOWN: "발행 결과 확인 필요", SCHEDULED_CONFIRMED: "예약 발행 확인",
};
const ACTIVE_STATUSES = new Set([
  "UPLOADING", "QUEUED", "QUEUE_NOTIFIED", "LOCAL_ACCEPTED", "QUEUED_LOCAL",
  "GENERATING", "APPROVED", "PUBLISHING",
]);
        ...
        {view === "create" ? (
          ...
          <>
            ...
            <section className="marketing-job-panel"><header><div><h2>최근 작업</h2><p>Cloudflare에 보관된 상태와 로컬 작업 ID를 함께 표시합니다.</p></div><span>최근 50건</span></header>
              {jobs.length ? <div className="marketing-job-list">{jobs.map((job) => <article key={job.id} className={job.status === "FAILED" ? "failed" : ""}><div className="marketing-job-icon"><Wrench aria-hidden="true" /></div><div className="marketing-job-main"><header><span className={`marketing-status status-${job.status.toLowerCase()}`}>{STATUS[job.status] || job.status}</span><time title="작업 등록 시각">등록 {formatDate(job.createdAt)}</time></header><h3>{job.district} · {job.symptom}</h3><p>{job.causeUnknown ? "원인 미확정" : job.diagnosedCause} · {job.actionsTaken}</p><p><small>{job.localJobId ? "마지막 서버 동기화" : "상태 갱신"}: {formatDate(job.updatedAt)}</small></p><small>{job.id}{job.localJobId ? ` → ${job.localJobId}` : ""}</small></div></article>)}</div> : <div className="integration-empty"><Wrench /><strong>아직 수리일지 작업이 없습니다.</strong><p>수리일지 등록 탭에서 현장 기록과 사진을 올려 주세요.</p></div>}
            </section>
          </>
        )}
      </section>
